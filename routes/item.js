        ...
        MarkService.findById(markId, function (err, mark) {
          if (err) {
            ...
            ApiUtils.api(req, res, ApiUtils.OK, null, {
              itemId: item._id,
              itemShortlink: item.shortlink,
              markShortlink: mark.shortlink,
              markId: item.markId
            });
          }
        });
...
exports.listUserPublic = function (req, res) {
  ApiUtils.auth(req, res, function (user) {
    ...
    ItemService.listUserPublic(username, longitude, latitude, user._id, function (err, data) {
      if (err) {
        ApiUtils.api(req, res, ApiUtils.SERVER_INTERNAL_ERROR, err, null);
      } else {
        ApiUtils.api(req, res, ApiUtils.OK, null, data);
...
exports.delete = function (req, res) {
  ApiUtils.auth(req, res, function (user) {
    var itemId = req.body.itemId;
    ItemService.delete(itemId, user._id, function (err) {
      if (err) {
        ApiUtils.api(req, res, ApiUtils.SERVER_INTERNAL_ERROR, err, null);
      } else {
        ApiUtils.api(req, res, ApiUtils.OK, null, null);
