...
function createItemCount1ItemInMark(item) {
  ...
  if (item.status == STATUS_OK) {
    MarkService.findById(item.markId, function(err, mark)
    {
      ...
      if (!u.isEqual(mark.userId, item.userId)) {
...
service.fillItem = function (item, longitude, latitude, userId, callback) {
  ...
  var ownerUserId = item.userId._id || item.userId;
...
service.delete = function (itemId, userId, callback) {
  service.findById(itemId, function (err, item) {
    if (err) return callback(err);
    if (!item) return callback("Not found")
    if (!u.isEqual(item.userId, userId)) return callback(Utils.error(Utils.ERROR_CODE_UNAUTHORIZED));

    item.status = STATUS_DELETED;
    item.save(function (err) {
      if (err) return callback(err);

      var promise = UserService.removePostHandler(item);
...
service.listByMarkRaw = function (markId, callback) {
  Item.find({markId: markId}, function(err, items) {
    if (err) return callback(err);
    callback(null, items);
  });
}
